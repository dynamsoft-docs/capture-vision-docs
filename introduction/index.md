...
Dynamsoft Capture Vision (DCV) is an aggregating SDK of a series of specific functional products, which cover object capturing, content understanding, result parsing, and interactive viewing.

1. To capture documents and labels, Dynamic Web TWAIN (DWT) and Dynamsoft Camera Enhancer (DCE) are to get images from scanners and cameras respectively.
...
3. In order to extract fields, the text/bytes result is parsed using Dynamsoft Code Parser (DCP).
4. To view, edit and output the results of the last steps, Dynamic Web TWAIN (DWT) and Dynamsoft Camera Enhancer (DCE) provide some basic viewing and editing functions. An advanced viewer is on the way. DCV is continuously being improved to incorporate these functional products.
...
| :---------------------------------------------------------------------------------| :-----------------|
| Reading barcodes.                                                                 | "dbr"             |
| Recognizing text on labels.                                                       | "dlr"             |
| Detecting document borders.                                                       | "ddn"             |
| Recognizing and parsing AAMVA driver's licenses.                                  | "aamva"           |
| Recognizing and parsing MRZ regions on various IDs, visas, and passports.         | "mrz"             |
...
Here are some common examples of dependencies between tasks:

| Requirements                      | Task dependencies     | Task execution order description                                               |
| :---------------------------------|:----------------------| :------------------------------------------------------------------------------|
| Recognize Driver's license        |  DCP=>DBR             |Read the PDF417 barcode => Parse the decoded bytes                              |
| Recognize text around the barcode |  DLR=>DBR             |Read the Barcode => Calculate the rough position of the text and recognize it   |
| Recognize passport MRZ            |  DCP=>DLR=>DDN        |Detect passport borders => Recognize MRZ text => Parse the MRZ text             |
